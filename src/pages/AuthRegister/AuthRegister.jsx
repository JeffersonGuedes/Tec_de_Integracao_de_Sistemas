...
function AuthRegister() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const data = await transformRequest('/api/v1/change_password/', 'POST', {
                old_password: currentPassword,
                new_password: password,
                confirm_password: passwordRepet,
            }, refresh);
            if (data.status)
                localStorage.setItem('notification', JSON.stringify({ color: 'success', text: 'Senha Atualizada com sucesso.' }));
            window.location.href = '/';
        } catch (error) {
            localStorage.setItem('notification', JSON.stringify({ color: 'danger', text: 'A senha atual está incorreta.' }));
            console.error(error);
            window.location.reload();
        } finally {
            setLoading(false);
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit} className="flex justify-center items-center h-screen mx-[7vw]">
                <CardLogin title="Redefinir minha senha" subtitle="Para redefinir sua senha, digite a senha enviada por e-mail."
                >
                    <InputForm
                        user={iconCpf}
                        forLabel="password"
                    ...
                    <ul className="text-[var(--color-gray-3)] ml-3 mb-3 list-inside list-disc">
                        <li className={hasMinLength ? 'text-green-500' : 'text-red-500'}>No mínimo 8 caracteres</li>
                        <li className={hasUppercase ? 'text-green-500' : 'text-red-500'}>Uma letra maiúscula</li>
                        <li className={hasLowercase ? 'text-green-500' : 'text-red-500'}>Uma minúscula</li>
                        <li className={hasNumber ? 'text-green-500' : 'text-red-500'}>Um Número</li>
                        <li className={hasSpecialChar ? 'text-green-500' : 'text-red-500'}>Um caractere especial.</li>
                    </ul>
                    <Btn
                    id="authRegister"
                    nameBtn={loading ? <ClipLoader color="#fff" size={20} /> : "Redefinir Senha"}
                    width='w-full'
                    height='h-[3.8rem]'
                    disabled={loading}
                    className="
