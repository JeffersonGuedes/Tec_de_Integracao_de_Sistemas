...
function AuthReset() {
    ...
    useEffect(() => {
        transformRequest('/api/v1/api/captcha/', 'GET')
            .then(data => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!rawValue || !captchaValue) {
            localStorage.setItem('notification', JSON.stringify({ color: 'danger', text: 'Por favor, preencha todos os campos.' }));
            return;
        } try {
            const data = await transformRequest('/api/v1/api/validate-captcha/', 'POST', {
            ...
            if (data.message === "Captcha válido") {
                try {
                    await transformRequest('/api/v1/auth_reset/', 'POST', {
                        ...
                        localStorage.setItem("notification", JSON.stringify({ color: "success", text: "Nova senha enviada para o e-mail cadastrado." }));
                        window.location.href = "/";
                    });
                } catch (error) {
                    localStorage.setItem("notification", JSON.stringify({ color: "danger", text: "Cpf inválido. Tente novamente." }));
            ...
            localStorage.setItem('notification', JSON.stringify({ color: 'danger', text: 'Captcha inválido. Tente novamente.' }));
            window.location.reload();
        }
        setLoading(false);
    }
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit} className="flex justify-center items-center h-screen mx-[10vw]">
                <CardLogin title="Esqueci minha senha" subtitle="Para redefinir sua senha, digite o seu CPF">
                    <InputForm
                        user={iconCpf}
                        forLabel="cpfInput"
                        textLabel="CPF"
                    ...
                    <div className="input-form flex flex-row w-full h-[5rem] bg-[var(--color-gray)] justify-center content-start rounded-[0.5rem] my-[1.5rem] p-[0.5rem] gap-[1rem]">
                        {captchaImage ? (
                            <img className="w-[30rem] h-[4rem]" src={captchaImage} alt="captcha" />
                        ) : null}
                        <button className="cursor-pointer" type="button" onClick={handleReloadCaptcha}><img className="w-[2rem]" src={Refresh} alt="image refresh" /></button>
                    </div>
                    <InputForm
                        user={iconSenha}
